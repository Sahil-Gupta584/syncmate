...
import { trpcProcedure, trpcRouter } from "../../server/trpc.ts";
import { dbActionsZodSchema } from "./schema.ts";
      ...
      try {
        ...
        const channel = await prisma.channel.create({
          data: {
            ...
            description:
              channelResponse.data.items[0].snippet.description ?? "",
  ...
  getEditorVideos: trpcProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        if (!isExists) throw new Error("Editor not found");
        return backendRes({ ok: true, result: isExists });
      } catch (error) {
