...
export async function getGoogleServices(userId, code) {
    try {
        const auth = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, code
        ...
        if (code) {
            ...
            const refresh_token = jwt.verify(account.refreshToken, process.env.AUTH_SECRET);
            auth.setCredentials({
...
export async function updateGoogleDrivePermissions({ gDriveId, selectedEditorEmails, videoOwnerId, }) {
    const { result } = await getGoogleServices(videoOwnerId); // or ownerId if needed
    if (!result)
        throw new Error("Failed to get Google services");
    const { drive } = result;
    ...
    const existingEmails = permissions.data.permissions?.map((p) => p.emailAddress) ?? [];
    const toGrant = selectedEditorEmails.filter((email) => !existingEmails.includes(email));
    const toRevoke = existingEmails.filter((email) => !selectedEditorEmails.includes(email));
    console.log("toGrant", toGrant);
    await Promise.all(toGrant.map((email) => drive.permissions.create({
        fileId: gDriveId,
        requestBody: {
            role: "writer",
            type: "user",
    ...
    await Promise.all(permissions.data.permissions
        .filter((p) => toRevoke.includes(p.emailAddress) &&
        p.emailAddress !== videoOwnerEmail)
        .map((p) => {
        if (p.emailAddress === videoOwnerEmail)
            return;
...
export async function updateThumbnails({ videos, ownerId, }) {
    try {
        ...
        for (const { videoId, gDriveId } of videos) {
            ...
            const res = await axios.post("https://api.imgbb.com/1/upload?key=b10b7ca5ecd048d6a0ed9f9751cebbdc", form);
